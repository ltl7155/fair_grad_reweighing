...
class GateLayer_mask(nn.Module):
    def __init__(self, input_features, output_features, size_mask, units=[]):
        super(GateLayer_mask, self).__init__()
        self.input_features = input_features
        self.output_features = output_features
        self.size_mask = size_mask
        mat = torch.ones(output_features)

        mat[units] = 0
#         print(mat)
        self.weight = nn.Parameter(mat)
